...
class GoPro:
    ...
    def __init__(self, camera="detect", ip_address="10.5.5.9", mac_address="AA:BB:CC:DD:EE:FF", debug=True, timeoutz=5, webcam_device="usb0", api_type=constants.ApiServerType.SMARTY):
        self._poweron_attempts = None
    ...
    def KeepAlive(self):
        ...
        max_loops = 100000  # example large cap
        loop_count = 0
        ...
        while True:
            ...
            if loop_count >= max_loops:
                print("Reached KeepAlive loop cap, exiting KeepAlive().")
                break
    ...
    def _prepare_gpcontrol(self, retries=4):
    ...
    def whichCam(self):
        """ This returns what type of camera is currently connected.
         - gpcontrol: HERO4 Black and Silver, HERO5 Black and Session, HERO Session (formerly known as HERO4 Session), HERO+ LCD, HERO+.
         - auth: HERO2 with Wi-Fi BacPac, HERO3 Black/Silver/White, HERO3+ Black and Silver. """
        counter = 0
        if self._camera != "":
            return self._camera
        else:
            try:
                ...
                # Exceptions: HX (HeroSession), FS (Fusion), HD3.02 (Hero+), H18 (Hero 2018), H19 (MAX), H21 (HERO10)
    ...
    def power_on(self, _mac_address=""):
        if not hasattr(self, "_poweron_attempts"):
            self._poweron_attempts = 0
        if self._poweron_attempts >= 3:
            print("Reached max power_on attempts. Skipping power_on call.")
            return
        self._poweron_attempts += 1
    ...
    def pair(self, usepin=True):
        ...
        The code is useless afterward. This function will pair your GoPro to the machine without the need of using the mobile app -- at all. """
        if usepin == False:
            paired_resp = ""
    ...
    def getInfoFromURL(self, url):
        ...
        media = [url.replace("http://" + self.ip_addr +
                             "/videos/DCIM/", "").replace("/", "-").rsplit("-", 1)[0],
                 url.replace("http://" + self.ip_addr +
